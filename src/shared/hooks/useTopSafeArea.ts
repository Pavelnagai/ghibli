...
export const useTopSafeArea = (offset = 0) => {
	...
	const [contentSafeAreaInsets, setContentSafeAreaInsets] = useState(() => viewport.contentSafeAreaInsets())
	...
	useEffect(() => {
		...
		const initVariables = () => {
			...
			root.style.setProperty('--safe-area-inset-top', `${safeAreaInsets.top + contentSafeAreaInsets.top}px`)
			root.style.setProperty('--safe-area-inset-bottom', `${safeAreaInsets.bottom + contentSafeAreaInsets.bottom}px`)
			root.style.setProperty('--safe-area-inset-left', `${safeAreaInsets.left + contentSafeAreaInsets.left}px`)
			root.style.setProperty('--safe-area-inset-right', `${safeAreaInsets.right + contentSafeAreaInsets.right}px`)
		}
